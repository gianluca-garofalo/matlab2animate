  ...
  add_toolbar: function () {
    var counter, page;
     ...
     if (this.ns_pos || !this.ie6)
     {
       ...
       help.setAttribute("style", "color: black; padding-right: 10px");
       ...
       contents.setAttribute("style", "color: black");
...
//        location.href = this.href;
  ...
  show_slide: function (slide) {
    //console.log(String("show_slide:") + slide.id);
    if(slide.custom_onshow)try{slide.custom_onshow();}catch(err){console.log("custom_onshow failed");};
  ...
  hide_slide: function (slide) {
    //console.log(String("hide_slide:") + slide.id);
    if(slide.custom_onhide)try{slide.custom_onhide();}catch(err){console.log("custom_onhide failed");};
...
// invoked via F key
  ...
  key_down: function (event) {
    ...
    {
      //w3c_slidy.bigger();
      // disable to make presenter use less problematic
      return w3c_slidy.cancel(event);
    }
  ...
  set_visibility_all_incremental: function (value) {
    ...
    if (value == "hidden")
    {
      while (node)
      {
        ...
        if(node.attributes["custom_onhide"]) {
            try {
                eval(node.attributes["custom_onhide"].value);
            } catch(err) {
                console.log("custom_onhide failed");
            }
        }
    ...
    else // value == "visible"
    {
      while (node)
      {
        ...
        if(node.attributes["custom_onshow"]) {
            try {
                eval(node.attributes["custom_onshow"].value);
            } catch (err) {
                console.log("custon_onshow failed");
            }
        }
        node = w3c_slidy.next_incremental_item(node);
      }
    ...
    if (node && node.nodeType == 1) {  // an element
      ...
      if(node.attributes["custom_onshow"]) {
          try {
            ...
            console.log("custom_onshow failed");
          }
      }
    }
